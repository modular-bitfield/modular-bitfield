...
// and `#[derive_const(Specifier)]` derive macro.
#[test]
...
mod const_ops_support {
    ...
    const MY_BITFIELD: MyBitfield = MyBitfield::new().with_a(true).with_b(2).with_c(14);

    const MY_BITFIELD_RESULT: Result<MyBitfield, modular_bitfield::error::OutOfBounds> =
        MyBitfield::new().with_d_checked(1_000_000);

    const MY_BITFIELD_BYTES: [u8; 4] = MY_BITFIELD.into_bytes();

    const MY_BITFIELD_RECONSTRUCTED: MyBitfield = MyBitfield::from_bytes(MY_BITFIELD_BYTES);

    const MY_BITFIELD_A: bool = MY_BITFIELD_RECONSTRUCTED.a();
    const MY_BITFIELD_B: Result<u8, modular_bitfield::error::InvalidBitPattern<u8>> =
        MY_BITFIELD_RECONSTRUCTED.b_or_err();
}
