        ...
        |test_cases| {
            repeat(|| {
                for (disc, bytes) in test_cases.iter().copied() {
                    black_box(GeneratedVariableEnum::from_discriminant_and_bytes(
                        disc, bytes,
                    ));
                }
            });
...
fn bench_handwritten_from_discriminant() {
    ...
    compare(
        ...
        |test_cases| {
            repeat(|| {
                for (disc, bytes) in test_cases.iter().copied() {
                    black_box(HandwrittenVariableEnum::from_discriminant_and_bytes(
                        disc, bytes,
                    ));
                }
            });
...
fn validate_correctness() {
    ...
    for (gen, hw) in test_cases {
        ...
        let gen_reconstructed =
            GeneratedVariableEnum::from_discriminant_and_bytes(disc, gen_bytes).unwrap();
        let _hw_reconstructed =
            HandwrittenVariableEnum::from_discriminant_and_bytes(disc, hw_bytes).unwrap();

        assert_eq!(
            gen, gen_reconstructed,
            "Generated round-trip failed for {:?}",
            gen
        );
    ...
    for disc in 0u8..=3u8 {
        ...
        assert_eq!(
            gen_size, hw_size,
            "size_for_discriminant mismatch for disc {}",
            disc
        );
    }
